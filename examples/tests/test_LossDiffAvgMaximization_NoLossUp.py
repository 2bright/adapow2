...
import hashlib
import json
from adapow2.x import LossDiffAvgMaximization_NoLossUp
...
def test_model(hp, md):
  optimizer = LossDiffAvgMaximization_NoLossUp(hp)
  # optimizer = OHS(
  #   LossDiffAvgMaximization_NoLossUp(hp),
...
model_dataset_samples = kv.compile({
  ...
  [
    # (['mnist_tf_demo'], 'mnist', 10, 128, 'l2'),
    # (['mnist_logreg', 'mnist_tf_demo'], 'mnist', 30, 128, 'l2'),
    (['mnist_logreg', 'mnist_tf_demo', 'mnist_2_layers', 'mnist_2_layers_l2', 'mnist_vae', 'mnist_mlp', 'mnist_2c2d'], 'mnist', 10, 128, 'l2'),
  ]
})
...
for md in model_dataset_samples:
  ...
  for hp in adapow2_hyperparameters:
    ...
    hp['history_state_path'] = 'data.LossDiffAvgMaximization_NoLossUp.' + md['model']
    print(hp)
    index += 1
